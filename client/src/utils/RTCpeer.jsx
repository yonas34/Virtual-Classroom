...
if (typeof window.RTCPeerConnection !== 'undefined') {
  RTCPeerConnection00 = window.RTCPeerConnection;
   ...
   var RTCPeerConnections = function (options) {
    var w = window;
...
if (!options.onAnswerSDP) return;
 ...
 console.log('offer-sdp', options.offerSDP);

 if(options.offerSDP){peer.setRemoteDescription(options.offerSDP.sdp).then(function() {
 ...
 var dataChannelDict = { };
 channel = peer.createDataChannel(options.channel || 'sctp-channel',
dataChannelDict);
console.log(channel);
 setChannelEvents();
 }
...
try {
 chromeVersion = !!navigator.mozGetUserMedia ? 0 : parseInt(navigator.userAgent.match(
/Chrom(e|ium)\/([0-9]+)\./ )[2]);
}
catch(e) {}
...
export default RTCPeerConnections;